...
lib.getInput(year, day).then((data) => {
    ...
    let startState = { x: start.x, y: start.y, facing: 'E', score: 0, tiles: [{ x: start.x, y: start.y }] };
        ...
        if(current.score <= bestFinish && lines[current.y][current.x] !== '#')
        {
            ...
            if(best >= current.score)
            {
                ...
                if(end.x === current.x && end.y === current.y) {
                    if(current.score < bestFinish)
                    {
                        bestFinish = current.score;
                        bestPaths = [current];
                    }
                    else if(current.score === bestFinish)
                    {
                ...
                else
                {
                    ...
                    for(let newFacing of rots) {
                        ...
                        {
                            options.push({ x: current.x, y: current.y, facing: newFacing, score: current.score + 1000, tiles: [...current.tiles] });
                        }
                    }
                    ...
                    if(lines[target.y][target.x] !== '#') {
                        ...
                        let newTiles = [...current.tiles, {x: target.x, y: target.y}];
                        ...
                        while(!stopped)
                        {
                            ...
                            else 
                            {
                                ...
                                else
                                {
                                    target = newTarget;
                                    targetScore++;
                                    newTiles.push(newTarget);
                                }
                            }
                        }
                        if(target.x !== current.x || target.y !== current.y) {
                            options.push({ x: target.x, y: target.y, facing: current.facing, score: targetScore, tiles: newTiles });
                        }
                    }
                }
            }
    ...
    {
        for(let tile of path.tiles) {
            let idx = `${tile.x},${tile.y}`;
            if(!found[idx]) {
                found[idx] = true;
                count++;
            }
