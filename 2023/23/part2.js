...
lib.getInput(year, day).then((data) => {
    ...
    queue.push({ at: start, atLabel: '1,0', steps: 0, visited: { '1,0': true } });
    ...
    while(queue.length) {
        ...
        let globalKey = `${current.x},${current.y},${Object.keys(current.visited).join(',')}`;
        ...
        //     globalVisited[globalKey] = current.steps;
            ...
            for(let target of current.at) {
                if(!current.visited[target.to]) {
                    let newVisited = { ...current.visited };
                    newVisited[target.to] = true;
