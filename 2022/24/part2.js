    ...
    while(queue.any()) {
        ...
        if(!visited[identifier]) {
            ...
            for(let direction in deltas) {
                ...
                if(targetX === destination.x && targetY === destination.y) {
                    if(phase === 2) {
                    ...
                    if(phase === 0) {
                        destination = { x: startX, y: 0 };
                    }
                    else  {
                        destination = { x: endX, y: maxY };
                    }
                    phase++;
                }
                if(((current.x === targetX && current.y === targetY) || (targetX > 0 && targetY > 0 && targetX < maxX && targetY < maxY)) && (!newGrid[targetX] || !newGrid[targetX][targetY])) {
                    queue.push({ x: targetX, y: targetY, steps: current.steps + 1 });
                }
            }
        }
